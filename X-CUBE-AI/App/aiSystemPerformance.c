...
static int aiTestPerformance(int idx, int16_t *input_values)
{
  int iter;
  ai_i32 batch;
  int niter;
  ...
  for (iter = 0; iter < niter; iter++) {
      ...
      /* Fill input tensors with predefined data */
      for (int i = 0; i < net_exec_ctx[idx].report.n_inputs; i++) {
          ...
          for (ai_size j = 0; j < AI_BUFFER_SIZE(&ai_input[i]); ++j) {
              ...
              // Assign the predefined value to the tensor data based on its type
              ...
              value_index++;  // Increment to the next value in the input_values array
          }
      }
